...
class BuildItem:
    ...
    def populate_from_wiki_data_equipment(self) -> bool:
        ...
        attack_range = None
        if self.infobox_version_number is not None:
            key = "attackrange" + str(self.infobox_version_number)
            attack_range = self.extract_infobox_value(bonuses_template, key)
        if attack_range is None:
            attack_range = self.extract_infobox_value(bonuses_template, "attackrange")
        if attack_range is not None:
            self.item_dict["weapon"]["attack_range"] = infobox_cleaner.caller(attack_range, "attackrange")
        else:
            # If not present, set to 0
            self.item_dict["weapon"]["attack_range"] = 0
